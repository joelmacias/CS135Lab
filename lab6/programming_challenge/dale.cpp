...
int main()
{
    ...
    cout << "Hello! This program allows you to compute the triangle number"
    << endl
    << "or factorial of an integer. Enter '!' to find the factorial"
    << endl
    << "or '+' to find the triangle number: ";
    cin >> bangOrPlus;
    ...
    if (bangOrPlus == '+') {
        ...
        cout << "Please enter an Integer that is less than 50: ";
        cin >> n;
        
        if (n < 50) {
            cout << factorial(n) << endl;
        } else {
            cout << n << " is not less then 50. Aborting program."
            << endl;
        }
    } else
        cout << endl << "'" << bangOrPlus << "'"
        << " is not a valid input. Aborting program."  << endl;
